...
start "Farm2Home Backend (FastAPI)" cmd /k "cd /d %~dp0backend && python -m uvicorn app.main:app --port 8000 --reload"

timeout /t 2 >nul

echo [2/2] Launching Frontend Web App on http://localhost:5173 ...
start "Farm2Home Frontend (Vite)" cmd /k "cd /d %~dp0frontend && npm run dev"
